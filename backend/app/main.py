...
async def alice_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if "ответь" in command and "отзыв" in command and ("все" in command or any(char.isdigit() for char in command)):
        try:
            ...
            print(f"Error replying to specific feedbacks for Alice: {e}")
            return _alice_response("Не удалось обработать запрос. Попробуй ещё раз.", end_session=False)
    ...
    if "сколько" in command and "отзыв" in command:
        try:
            ...
            print(f"Error getting feedbacks for Alice: {e}")
            return _alice_response("Не удалось получить данные об отзывах. Попробуй ещё раз.", end_session=False)
    ...
    if "сколько" in command and "вопрос" in command:
        try:
            ...
            print(f"Error getting questions for Alice: {e}")
            return _alice_response("Не удалось получить данные о вопросах. Попробуй ещё раз.", end_session=False)
    ...
    if ("процент" in command or "%" in command or "сколько" in command) and ("5" in command or "пять" in command or "пятизв" in command):
        try:
            ...
            print(f"Error getting 5-star stats for Alice: {e}")
            return _alice_response("Не удалось получить статистику. Попробуй ещё раз.", end_session=False)
    ...
    if ("какие" in command or "каких" in command) and "отзыв" in command and ("остал" in command or "есть" in command):
        try:
            ...
            print(f"Error getting remaining feedbacks for Alice: {e}")
            return _alice_response("Не удалось получить список отзывов. Попробуй ещё раз.", end_session=False)
    ...
    if "прочитай" in command and "отзыв" in command:
        try:
            ...
            print(f"Error reading feedbacks for Alice: {e}")
            return _alice_response("Не удалось прочитать отзывы. Попробуй ещё раз.", end_session=False)
